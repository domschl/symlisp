;; --- LET ---
(define-test "let-simple" (lambda () (assert-equal (let ((x 1)) x) 1)))
(define-test "let-two-vars" (lambda () (assert-equal (let ((x 1) (y 2)) (+ x y)) 3)))
(define-test "let-shadowing" (lambda () (assert-equal (let ((x 1)) (let ((x 2)) x)) 2)))
(define-test "let-outer-scope" (lambda () (assert-equal (let ((x 1)) (let ((y 2)) (+ x y))) 3)))
...
(define-test "let-empty-bindings" (lambda () (assert-equal (let () 5) 5)))
(define-test "let-multiple-body" (lambda () (assert-equal (let ((x 0)) (set! x (+ x 1)) x) 1)))
...
;; --- LET* ---
(define-test "let*-simple" (lambda () (assert-equal (let* ((x 1)) x) 1)))
(define-test "let*-sequential" (lambda () (assert-equal (let* ((x 1) (y x)) y) 1)))
(define-test "let*-sequential-expr" (lambda () (assert-equal (let* ((x 1) (y (+ x 1))) y) 2)))
(define-test "let*-shadowing" (lambda () (assert-equal (let* ((x 1) (x (+ x 1))) x) 2)))
(define-test "let*-empty-bindings" (lambda () (assert-equal (let* () 5) 5)))
(define-test "let*-multiple-body" (lambda () (assert-equal (let* ((x 0)) (set! x (+ x 1)) x) 1)))

;; --- LETREC* ---
(define-test "letrec*-simple" (lambda () (assert-equal (letrec* ((x 1)) x) 1)))
...
(define-test "letrec*-empty-bindings" (lambda () (assert-equal (letrec* () 5) 5)))
(define-test "letrec*-multiple-body" (lambda () (assert-equal (letrec* ((x 0)) (set! x (+ x 1)) x) 1)))
...
(define-test "builtin-variadic-list"
  (lambda ()
    (assert-equal (list) '())
...
(define-test "apply-no-intermediate-args"
  (lambda ()
    (assert-equal (apply + '(1 2 3 4)) 10)))

(define-test "apply-with-intermediate-args"
  (lambda ()
...
(define-test "apply-only-proc-and-empty-list"
  (lambda ()
    (assert-equal (apply + '()) 0)))

(define-test "apply-on-user-lambda"
  (lambda ()
    (let ((add (lambda (x y) (+ x y))))
      (assert-equal (apply add '(3 4)) 7))))

(define-test "apply-on-user-lambda-intermediate"
  (lambda ()
