...
(define-test "do-no-result-expr" ; Should return unspecified (NIL)
  (lambda ()
    (assert-equal
      (do ((i 0 (+ i 1)))
...
(define-test "do-multiple-result-exprs"
  (lambda ()
    (assert-equal
      (do ((i 0 (+ i 1)))
...
(define-test "do-no-bindings" ; Not standard, but test behavior if allowed
  (lambda ()
    (assert-equal
      (do ()
