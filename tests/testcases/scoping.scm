...
(define-test "scope-global-access" (assert-equal scope-outer-1 10))

;; --- Function Scope ---
(define (scope-func-1 x)
  (+ x scope-outer-1)) ; Access global

(define-test "scope-func-access-global" (assert-equal (scope-func-1 5) 15))

(define (scope-func-2 x)
  (define scope-inner-1 20) ; Define local
  (+ x scope-inner-1))

(define-test "scope-func-define-local" (assert-equal (scope-func-2 5) 25))
...
(define-test "scope-func-shadow-global-let" (assert-equal (scope-func-3 5) 55))
(define-test "scope-global-unaffected-by-shadow" (assert-equal scope-outer-1 10))

;; --- Let Scope ---
(define scope-outer-2 100)
(define-test "scope-let-access-outer"
  (assert-equal (let ((y 5)) (+ y scope-outer-2)) 105))

(define-test "scope-let-shadowing"
  (assert-equal (let ((scope-outer-2 200)) scope-outer-2) 200))
...
(define-test "scope-closure-1" (assert-equal (scope-add-5 3) 8))
(define-test "scope-closure-2" (assert-equal (scope-add-10 3) 13))
(define-test "scope-closure-independent" (assert-equal (scope-add-5 1) 6)) ; Ensure add-10 didn't affect add-5's x
...
(define-test "scope-set!-global-initial" (assert-equal scope-set-test-var 1))
(scope-set-func) ; Call function to modify global
(define-test "scope-set!-global-modified" (assert-equal scope-set-test-var 2))

(define-test "scope-set!-let-var"
  (assert-equal (let ((z 10))
                  (set! z 11)
                  z)
                11))
